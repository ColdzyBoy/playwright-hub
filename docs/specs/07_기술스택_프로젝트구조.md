...
| 백엔드 | Node.js | 20+ | 런타임 |
| 백엔드 | Fastify | 5+ | HTTP API 서버 |
| 백엔드 | fastify-sse-v2 | 4+ | SSE(Server-Sent Events) 스트리밍 (로그/진행도) |
| 백엔드 | BullMQ | 5+ | 작업 큐 |
...
│   │   ├── lib/
│   │   │   ├── api.ts
│   │   │   ├── sse.ts                    # 실행 SSE(EventSource) 훅
│   │   │   ├── sse-edit.ts               # 수정 세션 SSE 훅
│   │   │   └── auth.ts
│   │   │
│   │   ├── Dockerfile
│   │   ├── next.config.js
...
│       │   │   └── validation.ts
│       │   ├── sse/
│       │   │   ├── runSse.ts             # 실행 SSE 스트림 (fastify-sse-v2)
│       │   │   └── editSse.ts            # 수정 세션 SSE 핸들러
│       │   └── lib/
│       │       ├── prisma.ts
│       │       ├── redis.ts
│       │       ├── queue.ts
...
- 실행 중 stdout/stderr와 진행도 스냅샷을 Redis Pub/Sub을 거쳐 SSE 스트림으로 전달
...
> **구현 참고**: Next.js 14, React 18, react-diff-viewer-continued, recharts, TanStack Query v5, zustand, zod를 런타임 의존성으로 갖는다. SSE 수신은 브라우저 native `EventSource` API를 사용하므로 별도 라이브러리가 필요하지 않다. TypeScript 5 / Tailwind 3 / @types/react를 개발 의존성으로 갖는다.

### apps/api/package.json

> **구현 참고**: Fastify 5, `@fastify/cors`, `@fastify/helmet`, `@fastify/compress`, `@fastify/jwt`, `@fastify/sensible`, `@fastify/rate-limit`, `@fastify/static`, `@fastify/type-provider-zod`, `fastify-sse-v2`, BullMQ 5, ioredis, @prisma/client, dockerode, bcryptjs, zod, simple-git, `@anthropic-ai/claude-agent-sdk`를 런타임 의존성으로, TypeScript 5 / Prisma 5 / `tsx` 및 `vitest`를 개발 의존성으로 갖는다. HTTP 테스트는 Fastify 내장 `app.inject()`를 사용하므로 supertest는 필요 없다.
