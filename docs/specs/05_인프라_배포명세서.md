...
flowchart TD
    ...
    D --> E["5. stdout/stderr + progress 이벤트 → SSE 스트림 전달"]
    E --> F{"6. 컨테이너 종료 코드"}
...
워커는 로그 라인 외에도 `status`, `totalTests`, `completedTests`, `passedTests`, `failedTests`, `skippedTests`, `progressPercent`를 포함한 진행도 스냅샷을 Redis Pub/Sub을 거쳐 API/SSE 계층으로 전달한다.
